...
class ProjectsHandler(threading.Thread):
    ...
    def set_current(self, name=None, path=None):
        ...
        if (not name) and path:
            ...
            for proj_name in self.projects:
                if path == self.projects[proj_name].path():
                    name = proj_name
    ...
    def new_project(
        ...
        if name in self.projects.keys():
            ...
            project = Project(paths, name, proj_type, author,
                              start_time, dirs, files, apps)
            self.projects[project.name] = project
    ...
    def get_project_data(self, member, sort_by="Relevance"):
        ...
        if member == "files":
            ...
            data = self.projects[self.current].apps
        elif member == "notes":
            data = self.projects[self.current].notes
        elif member == "urls":
            data = self.projects[self.current].urls
        else:
            raise ValueError("undefined data member")
        return data
