...
async def run_powershell(cmd):
  return str(Popen(["powershell", cmd], stdout=PIPE).communicate() [0])

async def pipe_PS_to_List(cmd):
  out_str = await run_powershell(cmd)
  out = out_str.split("\\r\\n")
  out = out[2:len(out) - 3]
  out = [row.strip() for row in out]
  return out

async def get_PS_table(cmd, items):
  full_cmd = cmd + " | Format-Table " + ", ".join(items) + " -AutoSize | Out-String -Width 1024"
  ps_out = await run_powershell(full_cmd)
  rows_list = ps_out.split("\\r\\n")[1:-2]
  ...
  table_list = await reslash(table_list)
  return table_list

async def reslash(data_list, slash="\\"):
  for ind, element in enumerate(data_list):
    try:
      if type(element) == list:
        element = await reslash(element, slash)
