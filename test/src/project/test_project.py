...
class TestProjectClass:
    def test_load_with_proper_path(
        self,
        test_proj_path,
    ...
    def test_load_with_path_with_no_project(
        self,
        empty_proj_path,
    ...
    def test_load_with_rarian_without_data(
        self,
        empty_proj_path,
        create_uninitialized_project,
    ):
        ...
        with pytest.raises(FileNotFoundError) as err:
            ...
            assert err.value == "No such file or directory: " \
                + f"'{no_project_path}\\.rarian\\data.json'"
    ...
    def test_save(self, test_proj_path, load_project):
        ...
        proj.name = "new name"
        proj.save()
        with open(os.path.join(test_proj_path, ".rarian\\data.json"), 'r') \
                as data_file:
    ...
    def test_remove_sub_dir(
        ...
        assert os.path.join(empty_proj_path, "temp",
                            "tmp.txt") not in proj.files

    def test_update_relevance(self, load_project):
        proj = load_project
        """should update relevance according to calculations"""
        assert proj.update_relevance(
            0, 'UP') == 1 - proj.add_rate / (1 + proj.add_rate)
        assert proj.update_relevance(
            0.5, 'UP') == 1 - 0.5 * proj.add_rate / (0.5 + proj.add_rate)
        assert proj.update_relevance(
            0.9, 'UP') == 1 - 0.1 * proj.add_rate / (0.1 + proj.add_rate)
        assert proj.update_relevance(1, 'UP') == 1

        assert proj.update_relevance(0, 'DOWN') == 0
        assert proj.update_relevance(
            0.5, 'DOWN') == proj.forget_rate * 0.5 / (0.5 + proj.forget_rate)
        assert proj.update_relevance(
            0.9, 'DOWN') == proj.forget_rate * 0.9 / (0.9 + proj.forget_rate)
        assert proj.update_relevance(
            1, 'DOWN') == proj.forget_rate / (1 + proj.forget_rate)

    def test_update_relevance_with_times(self, load_project):
        proj = load_project
        """should update relevance according to calculations multiple times"""
        assert proj.update_relevance(
            0, 'UP', 1) == 1 - proj.add_rate / (1 + proj.add_rate)
    ...
    def test_update_file_relevance_with_default_times(
        self,
        load_project,
        mocker,
    ):
    ...
    def test_update_file_relevance_with_given_times(
        self,
        load_project,
        mocker,
    ):
    ...
    def test_files_update_open_on_new_file(
        ...
        assert proj.files.at[file_name, "App"] == "app"
        assert proj.files.at[file_name, "Relevance"] == proj.update_relevance(
            proj.update_relevance(0, 'UP'), 'UP')
        assert proj.files.at[file_name, "LastAccessTime"] == now
        assert proj.files.at[file_name, "LastWriteTime"] == now

    def test_files_update_open_not_on_removed_file(
        self,
        load_project,
        test_proj_path,
        add_new_file,
    ...
    def test_forget(self, load_project):
        ...
        proj.forget()
        assert proj.files["Relevance"].equals(relevance_values.apply(
            lambda rel: proj.update_relevance(rel, 'DOWN')))
