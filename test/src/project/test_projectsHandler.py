...
class TestProjectsHandler:
    ...
    # add "notes", "urls"
    @pytest.mark.parametrize("member", ["files", "apps"])
    def test_get_project_data_members(
        ...
        expected_data = getattr(
            proj_handler.projects[proj_handler.current], member)
    ...
    def test_stop(self, create_project_handler):
        proj_handler = create_project_handler
        assert proj_handler.running
        proj_handler.stop()
        assert proj_handler.running is False
